            ...
            <table class="table-auto w-full mt-6 border-collapse">
                <thead>
                    <tr class="bg-gray-100 text-gray-600 uppercase text-sm leading-normal">
                        ...
                        <th class="py-3 px-6 text-left">Nama Eselon</th>
                        <th class="py-3 px-6 text-left">Eselon</th>
                ...
                <tbody class="text-gray-600 text-sm font-light">
                    ...
                    <tr class="border-b border-gray-200 hover:bg-gray-50 transition duration-300">
                        <td class="py-3 px-6 text-left">{{ $eselon->id }}</td>
                        <td class="py-3 px-6 text-left">{{ $eselon->name }}</td>
                        <td class="py-3 px-6 text-left">{{ $eselon->eselon }}</td>
                        <td class="py-3 px-6 text-center">
                            <button wire:click="edit({{ $eselon->id }})"
                            ...
                            <button wire:click="delete({{ $eselon->id }})"
        ...
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">
                    {{ $isEditMode ? 'Edit Eselon' : 'Tambah Eselon' }}
                </h3>
                ...
                <form wire:submit.prevent="{{ $isEditMode ? 'update' : 'store' }}">
                    <div class="mb-4">
                        ...
                        <input type="text" wire:model="name"
                            ...
                            placeholder="Masukkan nama Eselon">
                        @error('name') <span class="text-red-500">{{ $message }}</span> @enderror
                    </div>

                    <div class="mb-4">
                        <label for="eselon" class="block text-gray-700 font-bold">Eselon</label>
                        <input type="text" wire:model="eselon"
                            class="form-input rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-200 w-full mt-1"
